...
`/opt/snabju/scripts/deploy-production.sh`.
...
3. Clone this repository into `/opt/snabju` as that user and check out `master`.
4. Copy `.env.production.example` to `/opt/snabju/.env` and set real production
